define(['app'], function (app) {
    app.controller("PreExamFeePaymentController", function ($scope, $http, $localStorage, $state, $stateParams, AppSettings, PreExaminationService, AssessmentService, MarksEntryService, $uibModal, PaymentService) {
        ...
        $scope.showPaymentDetails = function () {
            ...
            if ($scope.current_schemeid !== undefined && $scope.Student.id !== undefined && $scope.Student.id != '' &&  $scope.isBackLog == false) {
                ...
                var getAdmissionsubmod = PreExaminationService.getPayExamFee(authData.SysUserID, $scope.Student.id, $scope.current_schemeid);
