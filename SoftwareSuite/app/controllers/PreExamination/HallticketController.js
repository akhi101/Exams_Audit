define(['app'], function (app) {
    app.controller("HallticketController", function ($scope, $http, $localStorage, $state, $stateParams, AppSettings, MarksEntryService, $uibModal, PaymentService, PreExaminationService) {
        ...
        $scope.getHallTkt = function () {
            ...
            $scope.dob = '123';
			$scope.loading = true;
            var getHallticket = PreExaminationService.getHallticket($scope.pinNumber, $scope.dob, $scope.Student.id);
                ...
                if (resp.Table !== undefined) {
                    ...
                    if ($scope.Student.id == 1) {
                        if (resp.Table[0].ResponceCode == '200') {
							...
							$scope.addtninfo4 = resp.Table2[0].addtninfo4;
                            $scope.loading = false;
                        ...
                        if (resp.Table[0].ResponceCode == '200') {
                            ...
                            $scope.result = true;
                            if ($scope.Photo == '' || $scope.Photo == null) {
                                alert("Photo is not available, please upload photo from your college or examination center and download Hall Ticket.")
                                $scope.result = false;
                                $scope.LoadImg = false;
                            }
