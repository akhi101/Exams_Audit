define(['app'], function (app) {
    app.controller("DiplomaFeePaymentController", function ($scope, $http, $localStorage, $window, $state, PreExaminationService, $uibModal, PaymentService, AdminService) {
        ...
        $scope.ValidateCaptcha = function (Pin, StudtypeId) {
            ...
            var captcha = PreExaminationService.ValidateCaptcha($scope.SessionCaptcha, $scope.CaptchaText, Pin, StudtypeId, $scope.ExamMonthYear);
            ...
            if ($scope.allItemsSelectedthing == true) {
                ...
                if ($scope.isAllchecked == false) {
                    ...
                    for (var i = 0; i < $scope.studentSubData.length; i++) {
                        ...
                        //$scope.studentSubjectsSelected;
                        dataPay.Pin = $scope.studPin;
                        //for (var j = 0; j < $scope.ActiveSemesters.length; j++) {                        //    if ($scope.ActiveSemesters[j].semid == $scope.current_schemeid) {
                        //        dataPay.Semester = $scope.ActiveSemesters[j].semester;
                        //        Semester = $scope.ActiveSemesters[j].semester;
                        //        break;
                        //    }
